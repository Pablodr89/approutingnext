export const getGuitarras = async () => {
	const response = await fetch(`${process.env.API_URL}/api/guitarras/?populate=imagen`, { cache: 'no-store' });
...
export const getGuitarrasByUrl = async url => {
	const response = await fetch(`${process.env.API_URL}/api/guitarras?filters[url]=${url}&populate=imagen`, {
		cache: 'no-store',
	});
	const { data } = await response.json();
